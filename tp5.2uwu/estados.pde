class Estados {
  ...
  void game() {
    ...
    if ( !abeja[0].colision(helado.x, helado.y, helado.ancho, helado.alto)&&contadorTiempo >= 3*60) {
      ganar();
    } else if (abeja[0].colision(helado.x, helado.y, helado.ancho, helado.alto) && contadorTiempo < 3*60){
      perder();
    }
  }
  void ganar () {
